...
class SegmentationTraining(NetworkTraining):
    ...
    def initialise_loss(self):
        ...
        if self.dist_flag:
            self.loss_fctn_dist = weighted_combined_pyramid_loss({'loss_names':['hausdorff_loss'],
                                                                  'loss_weights':[1]})
    ...
    def compute_batch_loss(self, batch):
        ...
        if self.dist_flag:
            dist_loss= self.hd_weight*self.loss_fctn_dist(out, yb, distb=distb,mask=mask)
            # print("\rDistance loss: {:.3f}, Standard OvSeg loss: {:.3f}.".format(dist_loss.item()/self.hd_weight,normal_loss.item()),
            #       sep='', end='', flush=True)
            return (normal_loss+dist_loss)#/1.05
        return normal_loss
...
class SegmentationTrainingV2(SegmentationTraining):
    ...
    def initialise_loss(self):
        self.loss_fctn_normal = weighted_combined_pyramid_loss(**self.loss_params)
        # self.loss_fctn_dist = weighted_combined_pyramid_loss(['hausdorff_loss'],[1])
        if self.dist_flag:
            print(self.dist_flag, "SegTrainingV2")
            self.loss_fctn_dist = weighted_combined_pyramid_loss(['hausdorff_loss'],[1])
