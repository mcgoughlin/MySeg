...
from ovseg.preprocessing.generate_gt_dtm import generate_dtm
...
class SegmentationPreprocessing(object):
    ...
    def preprocess_raw_data(self,
        ...
        for f in ['images', 'labels', 'fingerprints','dist_maps']:
        ...
        for raw_ds in raw_ds_list:
            ...
            for i in tqdm(range(len(raw_ds))):
                ...
                if dist_flag and ('label' in data_tpl):
                    dist_flag = True
                else:
                    dist_flag = False
                ...
                dtm = generate_dtm(lb).astype(im_dtype)
                # print(lb.shape)
                if self.is_cascade():
                    prev_pred = lb[:-1]
                    lb = lb[-1:]
                ...
                fingerprint_keys = [key for key in data_tpl if key not in ['image', 'label', 'dist_map']]
                ...
                np.save(join(outfolder, 'dist_maps',scan),dtm)
