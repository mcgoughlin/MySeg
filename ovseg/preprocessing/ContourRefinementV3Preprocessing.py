...
class ContourRefinementV3Preprocessing(SegmentationPreprocessing):
    ...
    def __call__(self, data_tpl, preprocess_only_im=False, return_np=False):
        ...
        if return_np:
            mask = mask.cpu().numpy()
            xb_prep = np.concatenate([xb_prep[:self.n_im_channels+1],
                                      mask,
                                      xb_prep[-1:]])
        else:
            xb_prep = torch.cat([xb_prep[:self.n_im_channels+1],
                                 mask,
                                 xb_prep[-1:]])
    ...
    def preprocess_raw_data(self,
        ...
        for f in ['images', 'labels', 'fingerprints', 'regions', 'prev_preds']:
        ...
        for raw_ds in raw_ds_list:
            ...
            for i in tqdm(range(len(raw_ds))):
                ...
                np.save(join(outfolder, 'prev_preds', scan), pred)
