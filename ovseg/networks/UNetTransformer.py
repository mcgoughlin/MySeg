...
class UNetTransformer(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_sizes,
                 is_2d, filters=32, filters_max=32, n_pyramid_scales=None,
        ...
        expected_patch_size = 64
        spatial_dims = expected_patch_size/(2**(self.n_stages-1))
        num_channels = self.blocks_down[-1].conv1.out_channels
        dimension = int((spatial_dims**3)*num_channels)
        
        self.Tran1 = torch.nn.Transformer(d_model=dimension,dim_feedforward=256)
        
        print("#################")
        print("Transformer model has a dimensionality of {}.".format(dimension))
        print("#################")
    ...
    def forward(self, xb):
        ...
        spatial_dims = np.asarray(input_shape[-3:])/(2**(self.n_stages-1))
        num_channels = self.blocks_down[-1].conv1.out_channels
        num_zeros = np.product(spatial_dims).astype(int)*num_channels
        initiate_decoder = torch.zeros((xb_shape[0],num_zeros))
        
        if torch.cuda.is_available():
            initiate_decoder = initiate_decoder.cuda()
            
        xb = self.Tran1.forward(xb_vect,initiate_decoder).view(xb_shape)
...
if __name__ == '__main__':
    ...
    net_3d = get_3d_UNet(1, 2, 5, 2, 4).to(gpu)
