...
class ClassSegmentationBatchDataset(object):
    ...
    def _maybe_store_data_in_ram(self):
        ...
        if self.store_data_in_ram:
            ...
            for ind in tqdm(range(self.n_volumes)):
                ...
                prev_pred = np.load(path_dict[self.prev_pred_key]).astype(np.uint8)
                ...
                self.data.append((im, prev_pred, labels))
        ...
        if self.store_coords_in_ram:
            ...
            for ind in tqdm(range(self.n_volumes)):
                ...
                # now get the prev prev coordinates
                if self.store_data_in_ram:
                    prev_pred = self.data[ind][1]
                else:
                    prev_pred = np.load(self.vol_ds.path_dicts[ind][self.prev_pred_key])
            ...
            self.pp_coords_fol = os.path.join(self.vol_ds.preprocessed_path,
                                                'prev_pred_coordinates_'+self.bias)
            if not os.path.exists(self.bias_coords_fol):
                os.mkdir(self.bias_coords_fol)
            if not os.path.exists(self.pp_coords_fol):
                os.mkdir(self.pp_coords_fol)
            ...
            for ind, d in enumerate(self.vol_ds.path_dicts):
                ...
                if case not in os.listdir(self.bias_coords_fol):
                    ...
                    coords_lb = self._get_bias_coords(labels)
                    np.save(os.path.join(self.bias_coords_fol, case), coords_lb)
                    
                    prev_pred = np.load(d[self.prev_pred_key])
                    prev_pred = maybe_add_channel_dim(prev_pred)[0]
                    coords_pp = np.stack(np.where(prev_pred > 0)).astype(np.int16)
                    np.save(os.path.join(self.prev_pred_coords_fol, case), coords_pp)
                    
                else:
                    coords_lb = np.load(os.path.join(self.bias_coords_fol, case))
                    coords_pp = np.load(os.path.join(self.pp_coords_fol, case))
                
                # save which index has which fg class
                for i in range(self.n_fg_classes):
                    if coords_lb[i].shape[1] > 0:
                        self.contains_fg_list[i].append(ind)
                
                if coords_pp.shape[1] > 0:
                    self.contains_pp_list.append(ind)
    ...
    def _get_volume_tuple(self, ind=None):
        ...
        if load_from_ram:
            ...
            prev_pred = np.load(path_dict[self.prev_pred_key], 'r')
            labels = np.load(path_dict[self.label_key], 'r')
            volumes = [im, prev_pred, labels]
    ...
    def _get_random_volume_ind(self, biased_sampling):
        if biased_sampling:
            ...
            return np.random.choice(self.contains_pp_list), -1
    ...
    def __getitem__(self, index):
        ...
        if biased_sampling:
            ...
            if self.store_coords_in_ram:
                # loading from RAM
                coords = self.pp_coords_list[ind]
            else:
                # or hard drive
                case = os.path.basename(self.vol_ds.path_dicts[ind][self.label_key])
                coords = np.load(os.path.join(self.pp_coords_fol, case))
