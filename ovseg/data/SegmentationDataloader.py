...
class SegmentationBatchDataset(object):
    ...
    def __init__(self, vol_ds, patch_size, batch_size, epoch_len=250, p_bias_sampling=0,
                 ...
                 label_key='label', dist_map_key = 'dist_map',store_data_in_ram=False, return_fp16=True, n_max_volumes=None,
                 bias='fg', n_fg_classes=None,dist_flag = False, *args, **kwargs):
        self.vol_ds = vol_ds
    ...
    def _maybe_store_data_in_ram(self):
        ...
        if self.store_data_in_ram:
            ...
            for ind in tqdm(range(self.n_volumes)):
                ...
                if self.dist_flag:
                    dtm = np.load(path_dict[self.dist_map_key]).astype(self.dtype)
                    dtm = maybe_add_channel_dim(dtm)
                    self.data.append((im, labels, dtm))
                else:
    ...
    def _get_volume_tuple(self, ind=None):
        ...
        if load_from_ram:
            ...
            if self.dist_flag:
                dist_maps = np.load(path_dict[self.dist_map_key], 'r')
                volumes = [im, labels, dist_maps]
            else:
                volumes = [im, labels]
