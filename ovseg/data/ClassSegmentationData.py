...
from ovseg.data.ClassSegmentationDataloader import ClassSegmentationDataloader


class ClassSegmentationData(DataBase):
    ...
    def initialise_dataloader(self, is_train):
        if is_train:
            ...
            self.trn_dl = ClassSegmentationDataloader(self.trn_ds,
                                                      augmentation=self.augmentation,
                                                      **self.trn_dl_params)
        else:
            print('Initialise validation dataloader')
            try:
                self.val_dl = ClassSegmentationDataloader(self.val_ds,
                                                          augmentation=self.augmentation,
                                                          **self.val_dl_params)
